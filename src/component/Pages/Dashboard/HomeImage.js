...
const HomeImage = () => {
  ...
  const handleImageClick= async(event)=>{
    ...
    const response = await fetch(`https://api.imgbb.com/1/upload?key=b1535f27f9d4809ccba56e7f3ae76fea`, {
        method: 'POST',
        body: formData
    });
    ...
    return (
        <div className='mx-[30px] mt-5 flex flex-col'>
           ...
           <input name="homeImage" type="file" placeholder="Type here" className="input input-bordered w-full max-w-xs" />
           <button className='btn mx-10 ' type="submit">Post</button>
           </form>
        </div>
    );
};
